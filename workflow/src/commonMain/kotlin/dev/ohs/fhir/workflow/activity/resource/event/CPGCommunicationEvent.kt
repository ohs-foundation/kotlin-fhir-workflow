...
class CPGCommunicationEvent(resource: Communication) :
  CPGEventResource<Communication>(EventStatusCodeMapperImpl()) {
  ...
  override fun setStatus(status: EventStatus, reason: String?) {
    resource =
      resource.copy(
        status =
          Enumeration(
            value = Communication.EventStatus.fromCode(mapper.mapStatusToCode(status) ?: "unknown")
          ),
        statusReason =
